...
'''
Lista todas las estaciones que en el ultimo mes hayan superado los 30 grados celsius
'''
def get_active_stations_with_high_temp():
    one_month_ago = timezone.now() - timezone.timedelta(days=30)
    high_temp_stations = Data.objects.filter(
        measurement__name='Temperature',
        value__gt=30,
        time__gte=one_month_ago
    ).values('station').annotate(station_count=Count('station')).filter(station_count__gt=0)

    return JsonResponse({'high_temp_stations': list(high_temp_stations)})


'''
Lista todas las estaciones que en el ultimo mes hayan superado los 30 grados celsius
'''
def get_stations_with_high_temp(request):
    try:
        high_temp_stations = Station.objects.filter(
            data__measurement__name='temperatura',
            data__value__gt=30
        ).distinct()
        stations_data = [
            {
            ...
            for station in high_temp_stations
        ]
...
'''
Lista todas las estaciones que en un mes determinado que hayan superado los 30 grados celsius
'''
from django.http import JsonResponse
from django.db.models import Q
from datetime import datetime, timedelta
...
def stations_with_high_temp(request, *args, **kwargs):
        from_str = request.GET.get('from')
        to_str = request.GET.get('to')
        try:
            start = parse_datetime(from_str)
            end = parse_datetime(to_str)
        except:
            start = None
            end = None
        logger.debug(f"Start date: {start}, End date: {end}")

        high_temp_stations = Station.objects.filter(
            data__measurement__name='Temperatura',
            data__value__gt=30,
            data__time__range=(start, end)
        ).distinct()

        logger.debug(f"High temp stations query: {high_temp_stations.query}")

        stations_data = [
            {
                'station_id': station.id,
                'location': f'{station.location.city.name}, {station.location.state.name}, {station.location.country.name}',
                'last_activity': station.last_activity,
                'active': station.active
            }
            for station in high_temp_stations
        ...
        logger.debug(f"Stations data: {stations_data}")

        return JsonResponse(
	{'stations': stations_data}, safe=False)
