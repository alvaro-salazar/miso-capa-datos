...
from django.db.models import Avg, Max, Min, Sum, Case, When, FloatField
...
def stations_with_high_temp(request, *args, **kwargs):
        ...
        to_datetime = parse_datetime(to_str)

        from_datetime = from_datetime.replace(tzinfo=None)
        to_datetime = to_datetime.replace(tzinfo=None)

        start=datetime.strptime(from_str, "%Y-%m-%dT%H:%M:%SZ") 
        end=datetime.strptime(to_str, "%Y-%m-%dT%H:%M:%SZ") 

        high_temp_stations = Station.objects.filter(
            data__measurement__name='Temperatura',
            data__avg_value__gt=30.0,
            data__base_time__range=(start, end)
        ).distinct()
